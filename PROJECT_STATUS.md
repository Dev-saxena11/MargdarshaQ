...
|---|---|---|---|
| 1 | **Graph-based Network Model** — weighted directed/undirected graph, nodes/edges, **"dynamic weight update mechanism"** | 🟢 Done | Dynamic weight updates, time-varying rush hour curves, and traffic incident simulation fully implemented (`graph_model.py`, `dynamic_vrp.py`). |
| 2 | **Mathematical Formulation** — complete optimization model of the VRP/traffic routing problem: objective function, capacity/time-window/flow constraints, decision variables | 🟢 Done | Written out in full in `docs/FORMULATION.md`: objective function, soft penalty formulation, random-key encoding, MIP equations, quantum delta-potential-well rules, and jump-cap mechanics. |
...
| 2 | **Traffic is not actually dynamic.** *(FIXED)* `TrafficIncident` + `DynamicVRPSimulator` added. | Satisfied. Mid-route QPSO re-optimization evaluates static (stuck) vs dynamic (rerouted) fleet performance under live bottlenecks. |
...
| 4a | **No formal mathematical formulation document.** *(FIXED)* `docs/FORMULATION.md` added. | Satisfied. Complete optimization model, objective function, decision variables, constraint equations, quantum update mechanics, and hardware roadmap formally documented. |
